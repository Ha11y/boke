<script setup>

</script>
<template>
    <div class="container"> 

       <div class="logo"><img src="../../../assets/image/logo.jpeg" alt=""></div>
       <div class="text">欢迎来到我的博客</div>
       <div class="article"><li>所有文章</li></div>
       <div class="tag">
         <li @click="$router.replace('/web')"> 分类一</li>
         <li>分类二</li>
         <li>分类三</li>
         <li>分类四</li>
       </div>
       <div class="searchContainer">
        <form action="" class="search">
            <input type="text" name="" id="" placeholder="搜索">
            <svg t="1691057890229" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2476" width="20" height="20" ><path d="M955.069071 864.311021 740.015134 649.258107c-3.752464-3.751441-8.841366-5.860475-14.149255-5.860475-5.306866 0-10.395768 2.108011-14.149255 5.860475l-16.692171 16.692171-38.34226-38.34226c53.03796-59.810201 85.298711-138.442072 85.298711-224.478588 0-186.774871-151.952784-338.727655-338.727655-338.727655S64.527642 216.35456 64.527642 403.12943c0 186.775894 151.952784 338.728678 338.727655 338.728678 86.36909 0 165.276231-32.510438 225.170343-85.913718l38.303374 38.303374-17.34504 17.34504c-7.812943 7.813966-7.812943 20.48352 0 28.297486l215.051891 215.052914c3.753487 3.751441 8.841366 5.860475 14.149255 5.860475 5.306866 0 10.395768-2.108011 14.149255-5.860475l62.334697-62.334697C962.883037 884.794541 962.883037 872.124987 955.069071 864.311021zM104.546078 403.12943c0-164.709319 133.9999-298.709219 298.709219-298.709219s298.709219 133.9999 298.709219 298.709219S567.964616 701.839673 403.255297 701.839673 104.546078 567.838749 104.546078 403.12943zM878.585119 912.496463 691.829691 725.741036l34.036187-34.036187 186.755428 186.755428L878.585119 912.496463z" fill="#2c2c2c" p-id="2477"></path></svg>
        </form>
       </div>
    </div>
</template>


<style scoped>
.searchContainer{
   width: 100px;
   height: 100%;
   display: flex;
   justify-content: center;
   align-items: center;
   margin-left: 40px;
}
.searchContainer .search{
    display: flex;
    justify-content: center;
    align-items: center;
}
.searchContainer .search input{
    width: 150px;
    height: 25px;

}
.container{
    display: flex;
    width: 100%;
    /* background-color: #bee9de; */
    height: 70px;
}
.flex-grow{
    flex-grow: 2;
}
.logo{
    width: 70px;
    height: 70px;
    justify-content: center;
    align-items: center;
    display: flex;
    margin-left: 400px;
}
.logo img{
  width: 70px;
  height: 70px;
}
.text{
    width: auto;
    height: 70px;
    justify-content: center;
    align-items: center;
    display: flex;
    margin-left: 20px;
}
.article{
    width: auto;
    height: 70px;
    justify-content: center;
    align-items: center;
    display: flex;
    margin-left: 20px;
}
.article:hover{
    background-color: #aaa3a3;
    
}
.article li{
    list-style: none;
}

.tag{
    width: auto;
    height: 70px;
    justify-content: center;
    align-items: center;
    display: flex;
    margin-left: 20px;
}
.tag li{
    height: 70px;
    display: flex;
    justify-content: center;
    align-items: center;
    list-style: none;
    margin-left: 20px;
}
.tag li:hover{
    background-color: #aaa3a3;
}
</style>